...
import { MapPin, Phone, Mail, Mountain, ExternalLink, ChevronDown, Heart, Utensils, ShoppingBag, Dog, Leaf } from "lucide-react";
...
const Index = () => {
  ...
  const teamMembers = [
    { key: 'max', icon: Heart },
    { key: 'christa', icon: Utensils },
    { key: 'hubert', icon: Mountain },
    { key: 'petra', icon: Heart },
    { key: 'ina', icon: Leaf },
  ];

  const animals = [
    { key: 'dogs', icon: Dog },
    { key: 'cattle', icon: Mountain },
    { key: 'ponies', icon: Heart },
  ];

  const culinaryHighlights = [
    { key: 'kasnockerl', price: '€9,50' },
    { key: 'steirerkasnockerl', price: '€10,50' },
    { key: 'bratlbrot', price: '€5,50' },
    { key: 'suppen', price: 'ab €4,00' },
  ];

  const shopProducts = [
    { key: 'salbe', price: '€6,00' },
    { key: 'johanniskraut', price: '€8,50' },
    { key: 'marmelade', price: '€3,00' },
    { key: 'honig', price: '€6,50 - €16,50' },
    ...
    <div className="min-h-screen bg-background">
      ...
      <section className="relative h-screen flex items-center justify-center overflow-hidden">
          ...
          <CarouselContent>
            {heroImages.map((image, index) => (
              <CarouselItem key={index}>
                <div 
                  className="h-screen bg-cover bg-center bg-no-repeat animate-ken-burns"
        ...
        <div className="absolute bottom-8 left-1/2 -translate-x-1/2 z-10 scroll-indicator">
          <ChevronDown className="w-8 h-8 text-primary-foreground animate-pulse-soft" />
        </div>
      </section>
      ...
      <section id="about" className="section-padding px-4 bg-background">
        <div className="container mx-auto max-w-6xl">
          ...
          <div className="grid md:grid-cols-3 gap-6 mt-20">
            {['regional', 'tradition', 'quality'].map((feature, index) => (
              <AnimatedSection key={feature} animation="fade-in-up">
                <Card className="border-border/50 shadow-sm card-hover h-full" style={{ animationDelay: `${index * 100}ms` }}>
          ...
          <div ref={teamStagger.containerRef} className="grid md:grid-cols-2 lg:grid-cols-3 xl:grid-cols-5 gap-6">
            {teamMembers.map((member, index) => {
              ...
              return (
                <Card 
                  key={member.key} 
                  className={`border-border/50 shadow-sm card-hover transition-all duration-700 ${
                    teamStagger.visibleItems[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                  }`}
                >
                  <CardContent className="pt-6 text-center">
          ...
          <div ref={animalsStagger.containerRef} className="grid md:grid-cols-3 gap-8">
            {animals.map((animal, index) => {
              ...
              return (
                <Card 
                  key={animal.key} 
                  className={`border-border/50 shadow-lg card-hover overflow-hidden transition-all duration-700 ${
                    animalsStagger.visibleItems[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                  }`}
                >
      ...
      <section className="relative section-padding px-4 overflow-hidden">
        ...
        <div className="container mx-auto max-w-6xl relative z-10">
          ...
          <div ref={culinaryStagger.containerRef} className="grid md:grid-cols-2 lg:grid-cols-4 gap-6">
            {culinaryHighlights.map((dish, index) => (
              <Card 
                key={dish.key} 
                className={`bg-primary-foreground/95 border-0 shadow-xl card-hover transition-all duration-700 ${
                  culinaryStagger.visibleItems[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                }`}
              >
                <CardContent className="pt-6 text-center">
          ...
          <div ref={shopStagger.containerRef} className="grid md:grid-cols-2 lg:grid-cols-5 gap-6">
            {shopProducts.map((product, index) => (
              <Card 
                key={product.key} 
                className={`border-border/50 shadow-sm card-hover transition-all duration-700 ${
                  shopStagger.visibleItems[index] ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                }`}
              >
                <CardContent className="pt-6 text-center">
